...
if __name__ == '__main__':
    total = 100
    dados = asyncio.Queue() # fila (assíncrono)
    el = asyncio.get_event_loop() # Criando loop de eventos

    print(f'Computando {total * 2:.2f} dados.')

    el.run_until_complete(gerar_dados(total, dados)) # Executa o loop de eventos
    el.run_until_complete(gerar_dados(total, dados)) # Executa o loop de eventos
    el.run_until_complete(processar_dados(total * 2, dados)) # Executa o loop de eventos
    
    el.close() # Fechando o loop de eventos
